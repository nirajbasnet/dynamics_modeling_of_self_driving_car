    ...
    def norm_train_network(self, train_dataset, epochs):
        ...
        for epoch in range(epochs * sequence_length):
            ...
            while index + batch_size * sequence_length <= train_dataset.__len__():
                ...
                # We update the total loss across the sequence
                ...
                self.optimizer.step()
                # We also calculate the loss at the last step of the sequence as this gives truer long run loss
                loss = self.criterion(outputs[:, -1, :], labels[:, -1, :])
                # We update the loss at the last cell
            ...
            print('    Epochs Completed: %2d, MSE loss across the entire sequence: %.4f, MSE loss at last cell: %.4f'
                  % (np.floor(epoch / sequence_length), total_sequence_loss/iteration, last_cell_loss/iteration))
